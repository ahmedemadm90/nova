                            ...
                            <td class="col-md">
                                <div class="dropdown text-center">
                                    ...
                                    <ul class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton1">
                                        @can('Safety Unfixed Permits Manage')
                                            <li>
                                                <form action="{{ route('unfixed.security.permit.approve', $permit->id) }}"
                                                    method="post">@csrf<button
                                                        class="dropdown-item">{{ __('Approve') }}</button>
                                                </form>
                                            </li>
                                            <li>
                                                <form action="{{ route('unfixed.security.permit.reject', $permit->id) }}"
                                                    method="post">@csrf<button
